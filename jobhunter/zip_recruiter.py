...
def scraper_zip_recruiter(skill, city, pages):
    ...
    for page in range(pages):
        ...
        for card in cards:
            ...
            location = (
                card.find("a", "company_location").text.strip()
                if card.find("a", "company_location")
                else None
            )
            company_name = (
                card.find("div", "company_name_row").text.strip()
                if card.find("div", "company_name_row")
                else None
            )
            description = (
                card.find("p", "job_snippet").text.strip()
                if card.find("p", "job_snippet")
                else None
            )
            salary = (
                card.find("div", "value").text.strip()
                if card.find("div", "value")
                else None
            )
            apply = (
                card.find("div", {"class": "job_actions"}).find("a")["href"]
                if card.find("div", {"class": "job_actions"}).find("a")
                else None
            )
